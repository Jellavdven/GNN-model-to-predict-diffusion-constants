# The GNN model that will be trained 
...
class myPNA(torch.nn.Module):
    def __init__(self, data_list, in_channels, hidden_channels, num_layers):
        ...
        # last linear layer
        self.lin = Linear(hidden_channels + 1, 1)

    def forward(self, x, edge_index, edge_attr, batch, area_frac):
        ...
        if area_frac.dim() == 1:
            area_frac = area_frac.view(-1, 1)  # [num_graphs, 1]
        x = torch.cat([x, area_frac], dim=1)
        x = self.lin(x)
        return F.softplus(x)